...
User = (function () {
	...
	User.prototype.getIdentity = function (roomid) {
		...
		if (roomid) {
			...
			if (room && room.auth) {
				...
				if (room.isPrivate || room.isClanRoom) return Config.groups.default[room.type + 'Room'] + this.name;
			}
		}
	...
	User.prototype.filterName = function (name) {
		...
		name = toName(name);
		while (Config.groups.bySymbol[name[0]] || name[0] === Config.mutedSymbol || name[0] === Config.lockedSymbol) {
		    name = name.slice(1);
	    }
		return name;
	};
