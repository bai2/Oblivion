...
var BattleRoom = (function () {
		...
		if (this.rated) {
			...
			if (!rated.p1 || !rated.p2) {
				...
				if (winner && !winner.authenticated) {
					...
					this.sendUser(winner, "|raw|<strong>Tú Ganaste 1 PD</strong>");
				}
		...
		if (ticksLeft[0] && ticksLeft[1]) {
			if (inactiveSide === 0 || inactiveSide === 1) {
				...
				if (inactiveUser0 && (ticksLeft[0] % 3 === 0 || ticksLeft[0] <= 4)) {
					this.sendUser(inactiveUser0, '|inactive|' + inactiveUser0.name + ' tiene ' + (ticksLeft[0] * 10) + ' segundos restantes.');
				}

				var inactiveUser1 = this.battle.getPlayer(1);
				if (inactiveUser1 && (ticksLeft[1] % 3 === 0 || ticksLeft[1] <= 4)) {
					this.sendUser(inactiveUser1, '|inactive|' + inactiveUser1.name + ' tiene ' + (ticksLeft[1] * 10) + ' segundos restantes.');
				}
			}
	...
	BattleRoom.prototype.requestKickInactive = function (user, force) {
		if (this.resetTimer) {
			if (user) this.sendUser(user, '|inactive|El contador de inactividad ya está haciendo la cuenta atrás.');
			return false;
		}
		if (user) {
