...
var commands = {
    ...
    seen: function (target, room, user, connection) {
        ...
        if (!parse.chatData[toId(target)] || !parse.chatData[toId(target)].lastSeen) {
            return this.sendPm('The user ' + target.trim() + ' nunca ha sido visto en las salas.');
        }
        return this.sendPm(target.trim() + ' fue visto por última vez ' + parse.getTimeAgo(parse.chatData[toId(target)].seenAt) + ' ago, ' + parse.chatData[toId(target)].lastSeen);
    },

    arena: function (target, room, user) {
        if (!global.salt) global.salt = 0;
        salt++;
        this.sendReply(salt + '% Arenoso.');
    },

    whois: (function () {
        var reply = [
            "Un usuario más de Pokémon Hispano",
            "Un entrenador Pokémon muy competitivo",
            "Este si vale la pena, retalo",
            "Normalmente, es malo... BaleBergaLaBida",
            "El es bastante bueno jugando~",
            "Alguien mucho mejor que tú",
            "Una persona cúl",
            "Una persona Kawaii",
            "Olvidalo, es virgen",
            "Un Líder",
            "Un amigo de Smiler 1313",
            "Que? Donde está?... *Se esconde bajo la cama*",
            "Alguien muy, pero muy arenoso :(",
            "Un Seguidor muy leal a RamirOP",
            "MMM a veces juega bien, a veces mal~",
        ];
        ...
        return function (target, room, user) {
            ...
            if (target === 'Shake') message = 'Administrador de este server, actualmente uno de los jugadores mas experimentados que he conocido; líder indiscutible del clan End Game';
            if (target === config.userid()) message = 'Ese Soy YO! soy mucho mejor que los admins, no les digas porque se pondran arenosos <w<';
            if (target === 'Neon') message = 'Administrador de este server, persona bastante dedicada y seria, no lo molestes, te puede salir de mal humor...';
            if (target === 'stevoduhhero') message = 'STEVO DUH GOD DAMN HERO! Respect him!';
            if (target === 'Kizzu') message = 'Random, Random Everywhere~ Administrador de este servidor, actualmente retirado de Pokémon, aunque llego a su puesto muy rápido <_<';
            if (target === 'Billy') message = 'Yugi Cabrera, haxero, haxero!!! D= corran que vino billy trampas locas x_x';
             if (target === 'Osco') message = 'Leader del server, especialista en la tier Ubers. Poco paciente, mejor evitar la desesperación de este hombre:';
